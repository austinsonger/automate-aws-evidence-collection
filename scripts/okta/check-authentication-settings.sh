...
OUTPUT_FILE="$OUTPUT_DIR/$YEAR.okta-authentication-settings.json"

# Fetch the list of policies
POLICIES=$(curl -s -X GET "https://${OKTA_DOMAIN}/api/v1/policies" -H "Authorization: SSWS ${API_TOKEN}" -H "Accept: application/json")
...
echo "$POLICIES" | jq -c '.[]' | while read -r policy; do
  ...
  POLICY_WITH_RULES=$(jq -n --arg id "$POLICY_ID" --arg name "$POLICY_NAME" --arg type "$POLICY_TYPE" --argjson rules "$RULES" '{id: $id, name: $name, type: $type, rules: $rules}')
...
echo "Authentication settings for policies have been saved to $OUTPUT_FILE"
